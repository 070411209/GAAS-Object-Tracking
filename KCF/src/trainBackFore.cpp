...
public:
    ...
    int refineSegments(const Mat &img, Mat &mask, Mat &dst);
};
...
int RosDetect::refineSegments(const Mat &img, Mat &mask, Mat &dst)
{
    int ret = 0;
    int niters = 3;
...
int RosDetect::start()
{
    ...
    else if (nFrmNum > 1)
    {
        ...
        ret = refineSegments(tmp_frame, bgmask, out_frame);
